(ns drata.main
  "Kotodama WASM entrypoint for the Drata clean-room actor (L4) — Clojure port.
  ...
  facts (`drata.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"drata.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "drata-compat" :tier tier :entities entities} 200])
